"""Mandatory OpenAI Responses ingress transaction around the pre-request gate."""
...
def _media_part_to_response(part: MediaPart) -> dict[str, Any]:
    if not isinstance(part.source, Base64Source):
        raise OmniRouteGuardError("gated media was not normalized to bounded base64")
...
def _rebuilt_input(prepared: DownstreamPayload) -> list[dict[str, Any]]:
    ...
    if not content:
        raise OmniRouteGuardError("gated payload did not contain downstream input")
...
class ResponsesIngressGateway:
    ...
    async def invoke(self, payload: object, *, tenant_id: str) -> ResponsesGatewayResult:
        ...
        try:
            ...
            input_digest = digest_responses_payload(raw_payload)
            output_digest = digest_responses_payload(downstream_payload)
        ...
        except (ReceiptValidationError, OmniRouteGuardError, TypeError, ValueError, OverflowError):
            return _blocked(
        ...
        try:
            response = await self._adapter.invoke(sealed)
        except OmniRouteGuardError:
            return _blocked(
        ...
        except OmniRouteAdapterError as adapter_error:
            return ResponsesGatewayResult(
                status="upstream_error",
                response=None,
                gate_result=outcome.public,
                error=SafeError(code=adapter_error.code, message=adapter_error.safe_message),
                http_status=adapter_error.http_status,
            )
