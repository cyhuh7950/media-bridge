...
[[ "$(uname -s)" = Linux && "$(uname -m)" = x86_64 ]] || { printf 'linux-x64 verification requires 64-bit Linux x86_64.\n' >&2; exit 2; }
...
shopt -s nullglob
artifacts=("$artifact_dir"/media-bridge-runtime-*-linux-x64.tar.gz)
(( ${#artifacts[@]} == 1 )) || { printf 'ArtifactDirectory must contain exactly one linux-x64 runtime archive.\n' >&2; exit 2; }
...
if (manifest.schemaVersion !== 1 || !entry) throw new Error('Runtime manifest does not contain schema 1 linux-x64 metadata.');
...
const result = {
  ...
  runtimeVersion: manifest.artifacts['linux-x64'].version,
  artifactName,
  sha256,
  platform: 'linux-x64',
