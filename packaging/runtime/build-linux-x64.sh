...
if platform.system() != "Linux" or platform.machine().lower() not in {"x86_64", "amd64"} or struct.calcsize("P") != 8:
    raise SystemExit("linux-x64 runtime requires 64-bit Linux x86_64 Python.")
if (pathlib.Path(sys.base_prefix) / "conda-meta").is_dir():
    raise SystemExit("linux-x64 runtime builds require official CPython; Conda distributions are unsupported.")
PY
...
artifact_name="media-bridge-runtime-$version-linux-x64.tar.gz"
