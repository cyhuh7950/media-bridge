  ...
  return {
    ...
    artifacts: {
      'win32-x64': {
        version: '0.1.12',
        published: true,
        url: 'http://127.0.0.1:18080/media-bridge-runtime-0.1.12-win32-x64.tar.gz',
        sha256: 'a'.repeat(64),
        archive: 'tar.gz',
...
test('manifest loader rejects schema and package version mismatches', () => {
  ...
  assert.throws(() => loadRuntimeManifest({ manifestPath, packageVersion: '0.1.12' }), /schema/i);
  fs.writeFileSync(manifestPath, JSON.stringify({ ...publishedManifest(), packageVersion: '0.2.0' }));
  assert.throws(() => loadRuntimeManifest({ manifestPath, packageVersion: '0.1.12' }), /package.*version/i);
  fs.rmSync(tempRoot, { recursive: true, force: true });
});
...
test('manifest fails closed for unpublished, missing, or unsafe artifacts', () => {
  assert.throws(() => selectArtifact({
    ...
    packageVersion: '0.1.12', platform: 'win32', arch: 'x64',
  }), /not published/i);
  assert.throws(() => selectArtifact({
    manifest: publishedManifest(), packageVersion: '0.1.12', platform: 'linux', arch: 'x64',
  }), /not available/i);
  assert.throws(() => selectArtifact({
