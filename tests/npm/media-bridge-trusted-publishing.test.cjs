...
test('release workflow publishes the npm package through GitHub OIDC without a long-lived token', () => {
  ...
  assert.match(workflow, /- publish-runtime-release/);
  ...
  assert.match(workflow, /npm publish .*\.tgz --access public/);
  assert.match(workflow, /publish-npm-package:[\s\S]*?if:\s*github\.ref_type == 'tag' && github\.ref_name == format/);
...
test('trusted publish validates the package identity and release tag before publishing', () => {
  ...
  assert.match(workflow, /media-bridge-release-\$\{\{ needs\.prepare-release\.outputs\.version \}\}-\$\{\{ github\.sha \}\}/);
