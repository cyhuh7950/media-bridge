...
def test_control_settings_can_explicitly_enable_insecure_http_for_private_deployment(
    monkeypatch: object,
    tmp_path: Path,
) -> None:
    from _pytest.monkeypatch import MonkeyPatch
    ...
    monkeypatch.setenv("MEDIA_BRIDGE_SNAPSHOT_KEY_ID", "test-key")
    monkeypatch.setenv("MEDIA_BRIDGE_SNAPSHOT_PATH", str(tmp_path / "active.json"))
    monkeypatch.setenv("MEDIA_BRIDGE_CONTROL_ORIGIN", "http://control.test")
    monkeypatch.setenv("MEDIA_BRIDGE_CONTROL_HOST", "control.test")
    monkeypatch.setenv("MEDIA_BRIDGE_CONTROL_ALLOW_INSECURE_HTTP", "true")
