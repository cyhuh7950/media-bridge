...
export function TestLabPage({ role, csrfToken, resultTtlMs = RESULT_TTL_MS }: TestLabPageProps) {
  const { items: connections } = useAdminList(role === "viewer" ? null : "/connections");
  ...
  async function submit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    if (!writable || !connectionId || !routingProfileId || media === null || media.size < 1 || media.size > MAX_MEDIA_BYTES) { setError("입력값을 확인하세요."); return; }
    setError(null); setResult(null); setResultSource(null);
    try {
      const response = await adminRequest<unknown>("/test-lab/preview", { method: "POST", csrfToken, body: { connection_id: connectionId, routing_profile_id: routingProfileId, target_model: "auto", conversion_profile: "generic", user_request: request, media_type: media.type === "application/pdf" ? "pdf" : "image", filename: media.name, declared_mime: media.type, media_base64: await toBase64(media) } });
