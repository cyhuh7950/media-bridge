...
type AccessKey = { selector: string; name: string; scopes: string[]; revoked: boolean };
...
export function TestLabPage({ role, csrfToken, resultTtlMs = RESULT_TTL_MS }: TestLabPageProps) {
  ...
  const [accessKeySelector, setAccessKeySelector] = useState("");
  ...
  useEffect(() => {
    let active = true;
    void adminRequest<AccessKey[]>("/credentials").then((credentials) => {
      if (!active) return;
      const usable = credentials.filter((credential) => !credential.revoked && credential.scopes.includes("responses:invoke"));
      setAccessKeys(usable);
      setAccessKeySelector((current) => current || usable[0]?.selector || "");
    }).catch(() => { if (active) setAccessKeys([]); });
    return () => { active = false; };
  }, []);
  ...
  async function submitOmniRoute(event: SyntheticEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!writable || !routingProfileId || media === null || !omniRouteEndpoint || !omniRouteApiKey || (accessKeys.length > 0 && !accessKeySelector) || media.size < 1 || media.size > MAX_MEDIA_BYTES) {
      setError("라우팅, 파일, OmniRoute endpoint와 API key를 확인하세요.");
      return;
    }
    setError(null); setResult(null);
    try {
