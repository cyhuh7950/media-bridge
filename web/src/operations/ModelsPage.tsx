...
export function ModelsPage({ role, csrfToken }: OperationsProps) {
  ...
  return <section aria-labelledby="models-title">
    <div className="page-heading"><div><h1 id="models-title">모델 관리</h1><p>Media Bridge가 외부에 제공할 OpenAI 호환 모델을 생성합니다. Provider 등록만으로는 공개되지 않습니다.</p></div>{writable ? <button type="button" onClick={openCreate}>모델 생성</button> : null}</div>
