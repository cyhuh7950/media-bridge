...
export function ProvidersPage({ role, csrfToken }: OperationsProps) {
  ...
  function openEdit(item: Record<string, unknown>) {
    ...
    setEditingId(textField(item, "id")); setDialogMode("edit"); setName(textField(item, "name")); setAlias(textField(item, "alias")); setKind(textField(item, "kind") === "llm" ? "llm" : "analysis"); setCatalogId(textField(item, "catalog_id")); setModelId(savedModel); setProtocol(textField(item, "protocol")); setCapabilities(Array.isArray(item.capabilities) ? item.capabilities.filter((value): value is string => typeof value === "string") : []); setEndpoint(textField(item, "endpoint")); setReasoningEffort(asReasoningEffort(item.reasoning_effort)); setApiKey(""); setSaveFailed(false);
  }
  ...
  async function submit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body: ProviderWriteRequest & Record<string, unknown> = { name, alias: alias || undefined, kind, catalog_id: catalogId && catalogId !== "__custom__" ? catalogId : undefined, model_id: modelId || undefined, endpoint, protocol: protocol || undefined, capabilities, reasoning_effort: kind === "llm" ? reasoningEffort : undefined, secret_ref: { kind: "db", identifier: "provider_api_key" }, api_key: apiKey || undefined, enabled: true };
  ...
  return (
    <section aria-labelledby="providers-title">
      ...
      {items ? <>
        ...
        <table><thead><tr>{writable ? <th><input aria-label="전체 Provider 선택" type="checkbox" checked={allSelected} onChange={(event) => { setSelectedIds(event.target.checked ? items.map((item) => textField(item, "id")) : []); }} /></th> : null}<th>이름</th><th>종류</th><th>엔드포인트</th><th>상태</th>{writable ? <th>작업</th> : null}</tr></thead>
      ...
      {dialogMode ? <section className="dialog-backdrop" role="dialog" aria-modal="true" aria-labelledby="provider-dialog-title"><form className="dialog-card form-grid" onSubmit={(event) => { void submit(event); }}><h2 id="provider-dialog-title">{dialogMode === "create" ? "Provider 등록" : "Provider 수정"}</h2>
        ...
        <label htmlFor="provider-operation-alias">Provider 약어</label><input id="provider-operation-alias" value={alias} onChange={(event) => { setAlias(event.target.value); }} placeholder="자동 생성되며 수정할 수 있습니다" required />
