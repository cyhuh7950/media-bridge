  ...
  return (
    <section aria-labelledby="providers-title">
      ...
      {dialogMode ? <section className="dialog-backdrop" role="dialog" aria-modal="true" aria-labelledby="provider-dialog-title"><form className="dialog-card form-grid" onSubmit={(event) => { void submit(event); }}><h2 id="provider-dialog-title">{dialogMode === "create" ? "Provider 등록" : "Provider 수정"}</h2>
        ...
        <ProviderCatalogPicker kind={kind} value={catalogId} onChange={(entry: ProviderCatalogEntry | null) => { setCatalogId(entry?.provider_id ?? ""); setName(entry?.display_name ?? ""); setEndpoint(entry?.default_endpoint ?? ""); setProtocol(entry?.protocol ?? ""); setCapabilities(entry?.capabilities ?? []); setReferenceKind("env"); setReference(entry?.secret_env ?? ""); }} />
