...
상태: 설계 초안 — 신산님이 모델별 설정 방향을 승인했고, 본 문서의 검토 및 구현 승인은 남아 있다.

## 1. 목적과 완료 기준

배포형과 설치형 Media Bridge에서 LLM Provider/모델별 추론 등급을 설정하고, 그 설정을 실제 Provider API 요청에 반영한다. 화면에 설정값만 보이는 것으로 끝나지 않고 DB 또는 설치형 정본 설정에서 저장·재시작 후 복구되며, Gateway가 지정 모델에 맞는 API 필드로 변환해야 한다.

완료 시 다음을 만족한다.

- 배포형 관리 화면과 설치형 설정 화면에서 현재 LLM 모델의 추론 설정을 조회·수정할 수 있다.
...
- 배포형 Provider 카탈로그에는 OpenAI Responses, OpenAI Chat Completions, Gemini native, Anthropic Messages, Upstage Solar 및 OpenAI 호환 Ollama 등이 있다.
...
- Data Plane은 서명 snapshot 기반으로 동작하므로 배포형 설정은 DB 저장만으로 실행에 반영되지 않는다. snapshot 발행 및 검증·런타임 구성까지 동일 계약을 유지해야 한다.
...
- 배포형 정본은 기존 DB Provider 설정이다. 선택값은 DB에 저장되고, 발행된 snapshot에 포함되어 Data Plane으로 전달된다. Secret 원문은 기존 credential 경로만 사용하며 snapshot에 넣지 않는다.
...
- 두 UI의 Non-Vision LLM 설정에 `추론 등급` selector를 제공한다.
...
| --- | --- | --- |
| OpenAI Responses | `reasoning.effort` 사용 | 선택 등급은 해당 model ID가 지원하는 범위만 노출 |
| OpenAI Chat Completions / Upstage Solar | API가 지원하는 `reasoning_effort` 사용 | Solar Pro 4는 기본으로 추론을 하므로 기본값은 필드 생략; 다른 Solar/모델은 검증된 계약만 노출 |
| Gemini native GenerateContent | Gemini 3 계열은 `thinkingConfig.thinkingLevel`, 2.5 계열은 `thinkingConfig.thinkingBudget`로 변환 | 버전별 지원 등급과 budget 범위를 적용; thinking을 끌 수 없는 모델에서 `없음` 미노출 |
| Anthropic Messages | adaptive-thinking `output_config.effort` 또는 해당 모델의 지원 계약 사용 | legacy budget 기반 모델과 adaptive 모델을 구분; 지원되지 않는 전환을 추정하지 않음 |
| Ollama native chat | `think` boolean/level로 변환 | GPT-OSS 등 모델별 강제 형식과 thinking 비활성화 가능 여부를 적용 |
| 기타·custom 호환 Provider | 명시적으로 등록·검증된 adapter가 있을 때만 등급 전송 | 알 수 없는 계약은 `Provider 기본값`만 허용 |
...
- OCR, Vision 분석 모델과 외부 client별 요청 옵션은 제외한다.
- 이 문서는 구현 명세 검토용이다. 사용자 검토·승인 후 별도 구현 계획을 작성하고, 해당 계획 승인 및 DB migration/배포 승인 경계를 확인한 뒤에만 코드를 수정한다.
