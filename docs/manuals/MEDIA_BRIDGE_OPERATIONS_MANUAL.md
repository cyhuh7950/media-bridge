...
API 키를 입력하면 키 원문은 DB에 암호화되어 저장되고 목록에는 다시 표시되지 않습니다. DB에 저장된 Provider를 수정할 때는 내부 식별자를 노출하지 않고 `DB에 저장된 API 키 사용 중`으로 표시합니다. 환경변수 참조를 직접 사용하는 경우에만 `PROVIDER_API_KEY`처럼 대문자와 숫자, 밑줄을 사용합니다.
