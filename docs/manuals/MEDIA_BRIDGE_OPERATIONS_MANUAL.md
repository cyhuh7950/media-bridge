...
API 키를 입력하면 키 원문은 DB에 암호화되어 저장되고 목록에는 다시 표시되지 않습니다. DB에 저장된 Provider를 수정할 때 보이는 `provider_api_key`는 사용자가 입력하는 환경변수 이름이 아니라 DB 저장 위치를 나타내는 자동 표시값이며 읽기 전용입니다. 환경변수 참조를 직접 사용하는 경우에만 `PROVIDER_API_KEY`처럼 대문자와 숫자, 밑줄을 사용합니다.
